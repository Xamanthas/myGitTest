...
{
    struct UX_OHCI_ED_STRUCT *eds[32];
    USHORT          frame_number;
    USHORT          reserved1;
    struct UX_OHCI_TD_STRUCT *done_head;
    UCHAR           reserved2[116];
} UX_HCD_OHCI_HCCA;
...
{
    struct UX_HCD_STRUCT  *hcd_owner;
    struct UX_HCD_OHCI_HCCA_STRUCT *hcca;
    ULONG           *hcor;
    UINT            nb_root_hubs;
    struct UX_OHCI_TD_STRUCT *done_head;
    struct UX_OHCI_ED_STRUCT *ed_list;
    struct UX_OHCI_TD_STRUCT *td_list;
    struct UX_OHCI_ISO_TD_STRUCT *iso_td_list;
} UX_HCD_OHCI;
...
{
    ULONG  dw0;
    struct UX_OHCI_TD_STRUCT *tailp_pa;
    struct UX_OHCI_TD_STRUCT *headp_pa;
    struct UX_OHCI_ED_STRUCT *next_pa;
    struct UX_OHCI_ED_STRUCT *prev_va;
    ULONG  status;
    struct UX_ENDPOINT_STRUCT *ux_endpoint;
    ULONG  frame;
} UX_OHCI_ED;
...
typedef struct UX_OHCI_TD_STRUCT
{
    ULONG           dw0;
    UCHAR           *cbp_pa;
    struct UX_OHCI_TD_STRUCT *next_pa;
    UCHAR           *be_pa;
    ULONG           reserved_1[4];
    struct UX_TRANSFER_STRUCT *request;
    struct UX_OHCI_TD_STRUCT  *next_va;
    struct UX_OHCI_ED_STRUCT  *ed_va;
    ULONG           length;
    ULONG           status;
    ULONG           reserved_2[3];
} UX_OHCI_TD;
...
#define UX_OHCI_TD_NO_INT                                   (0x7 << 21)
...
VOID    _ux_hcd_ohci_dump_registers(UX_HCD_OHCI *ohci);
VOID    _ux_hcd_ohci_dump_ed(UX_OHCI_ED *ed);
VOID    _ux_hcd_ohci_flush_ed(UX_OHCI_ED *ed);
UINT    _ux_hcd_ohci_stop(void);
UINT    _ux_hcd_ohci_destroy(void);
