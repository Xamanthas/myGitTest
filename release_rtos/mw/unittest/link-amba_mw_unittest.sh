...
show_variables()
{
        ...
        echo "---- show_variables() ----"
        echo ""
        echo "cmd_link-amba_mw_unittest: ${0} ${1} ${2} ${3} ${4}"
        echo "arg0=${0} arg1=${1} arg2=${2} arg3=${3} arg4=${4}"
        echo ""
        echo "---- Global variable ----"
        echo "LD=${LD}"
        echo "LDS_amba_mw_ut=${LDS_amba_mw_ut}"
        echo "LDFLAGS=${LDFLAGS}"
        echo "LDFLAGS_amba_mw_ut=${LDFLAGS_amba_mw_ut}"
        echo "srctree=${srctree}"
        echo "objtree=${objtree}"
        echo "DIR_amba_mw_ut=${DIR_amba_mw_ut}"
        echo ""
        echo "---- Local variable ----"
...
if [ "${CONFIG_ENABLE_VA_ADAS}" = "y" ]; then
        AMBA_LIBS="${AMBA_LIBS} libva_adas.a"
...
if [ "${CONFIG_BUILD_FOSS_OPENCV}" = "y" ]; then
        ...
        AMBA_LIBS="${AMBA_LIBS} libopencv_objdetect.a"
...
AMBA_VER_FILE="${DIR_amba_mw_ut}/${__AMBA_VER_STEM}.c"
AMBA_VER_OBJ="${DIR_amba_mw_ut}/${__AMBA_VER_STEM}.o"
...
RPC_CRC_OUTPUT="${DIR_amba_mw_ut}/AmbaIPC_RpcHeaderCrc"
RPC_CRC_TYPE="public"
...
RPC_CRC_PRV_OUTPUT="${DIR_amba_mw_ut}/AmbaIPC_PrivateRpcHeaderCrc"
...
LINK_LDS="${LDS_amba_mw_ut}"
LINK_LDFLAGS="${LDFLAGS} ${LDFLAGS_amba_mw_ut} ${__LINK_LIBS_INC}"

if [ ${KBUILD_AMBA_OUT_DIR} ]; then
        LINK_AMBA_OUT_DIR=${KBUILD_AMBA_OUT_DIR}
        mkdir -p ${LINK_AMBA_OUT_DIR}
else
        LINK_AMBA_OUT_DIR=${DIR_amba_mw_ut}
fi
